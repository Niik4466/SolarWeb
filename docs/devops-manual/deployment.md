...
3.  **Configuración de red:**
      * El **Nodo de Despliegue** debe tener acceso de red al **Nodo de Datos**.
      * Las variables de entorno en el Nodo de Despliegue (`INFLUX_ENDPOINT`, `MINIO_ENDPOINT`) deben apuntar a las direcciones IP o nombres de host del Nodo de Datos.
...
2.  **Crear el archivo `.env`:**
    Copiar las variables de entorno relacionadas con InfluxDB y MinIO en un archivo `.env` en la raíz del proyecto. Configurar los endpoints de InfluxDB y MinIO como 
    - `INFLUX_ENDPOINT=http://influxdb-solarweb:8086`
    - `MINIO_ENDPOINT=http://minio-solarweb:9000`
    ...
    > **Nota:** Exceptuando los endpoints y directorios de "upload", las variables definidas en el .env deben coincidir con las que el Nodo de Despliegue utilizará para conectarse.
...
2.  **Crear el archivo `.env`:**
    Copiar las variables de entorno relacionadas con InfluxDB y MinIO en un archivo `.env` en la raíz del proyecto. Las variables `INFLUX_ENDPOINT` y `MINIO_ENDPOINT` deben apuntar a la dirección IP o el nombre de host del Nodo de Datos.

      * Ejemplo: `INFLUX_ENDPOINT=http://<IP_NODO_DATOS>:8086`, `MINIO_ENDPOINT=http://<IP_NODO_DATOS>:9000`
